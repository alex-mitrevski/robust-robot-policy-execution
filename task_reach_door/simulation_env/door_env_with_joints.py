...
class ReachDoor(DoorEnv, gymnasium.Env):
        ...
        self.observation_space = spaces.Dict(
            {
                "image": spaces.Box(
                    low=0,
                    high=255,
                    shape=tuple(self.config["observation"]["image_shape"]),
                    dtype=np.uint8,
                ),
                "vector": spaces.Box(
    ...
    def apply_color_randomization(self):
        ...
        p.changeVisualShape(
            self.door,
            self.config["visual_shape"]["lever"],
            rgbaColor=random_lever_color,
        )
        p.changeVisualShape(
            self.door, self.config["visual_shape"]["door"], rgbaColor=random_door_color
        )
        p.changeVisualShape(
            self.door,
            self.config["visual_shape"]["protrusion"],
            rgbaColor=random_protrusion_color,
        )
        p.changeVisualShape(
    ...
    def step(self, action):
        ...
        denorm_action = []
        for num, low, high in zip(
            action,
            self.config["action"]["lower_limits"],
            self.config["action"]["upper_limits"],
        ):
        ...
        reward = (
            distance_reward * self.config["reward"]["distance_reward_multiplier"]
        ) + success_reward
    ...
    def reward_success(self):
        ...
        if (
            (
                np.linalg.norm(self.target_pos1 - current_end_effector_state[4])
                < dist_threshold
            )
            or (
                np.linalg.norm(self.target_pos2 - current_end_effector_state[4])
                < dist_threshold
            )
            or (
                np.linalg.norm(self.target_pos3 - current_end_effector_state[4])
                < dist_threshold
            )
        ):
            success = True
    ...
    def _get_state(self):
        ...
        kinova_state = np.concatenate(
            (current_end_effector_pose, current_joint_positions)
        )
